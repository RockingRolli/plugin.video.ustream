...
def get_video_info(href):
    if 'highlight/' in href:
        ...
        if amf_data['offairContent'].has_key('videos'):
            ...
            video_id = href.split('/')[1] #this should be the full video_id not the highlight
    else:
    ...
    if data['results'] is not None:
        for i in data['results']:
            ...
            if not stream_title is None:
                ...
                addLiveLink('%s [%s]' % (title, i['status']), i['id'], 4, icon, False)
    else:
        try:
...
def user_search(user=None):
    if user is None:
        ...
        if len(user) < 1:
            return
    debug_here()
    r = requests.get('http://api.ustream.tv/json/user/recent/search/username:like:%s?key=%s' % (user, dev_key))
    data = r.json()
...
def addLiveLink(name, url, mode, iconimage, showcontext=True):
    ...
    if showcontext:
        contextMenu = [('List all channels from this user',
                        'XBMC.Container.Update(%s?url=%s&mode=3)' % (sys.argv[0], urllib.quote_plus(url)))]
